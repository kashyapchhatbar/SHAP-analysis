...
def predict(input_values):
    input_values = torch.tensor(input_values, dtype=torch.float32, device=device)
    return final_model(input_values).detach().cpu().numpy()
...
if __name__ == "__main__":
    ...
    with torch.no_grad():
        e = shap.KernelExplainer(model=predict, data=X_train_summary)
    
    # e = shap.KernelExplainer(final_model, X_train_summary)
    shap_values = e.shap_values(X_test_numpy)[:, :, mark_id]
